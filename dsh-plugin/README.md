# dsh-novelnovel · NovelNovel 的 DeepSeek Harness 插件

把 NovelNovel（原本是一个浏览器里的 AI 小说写作 IDE）改成 DSH 插件：小说以**工作区里的普通文件**存在，
agent 直接用工具写、改、检索、导出；原本由应用自己调 LLM 的「续写」环节改由 harness 承担，
插件负责把作者的设定（世界观 / 命中词条 / 参与角色 / 写作预设 / 前文摘录）组装成一份写作简报。

浏览器应用仍然保留，两者共用同一份纯逻辑（`../src/lib`）：角色卡解析、预设解析、提示词组装、
词条关键词匹配、章节排序、全文搜索、PNG 卡读写、字数统计。

## 安装

```bash
# 1. 构建插件产物（lib/ 不入库，必须构建；改了 src/ 也要重建）
npm run build:plugin

# 2. 装进一个 dsh profile（不存在会自动初始化）
dsh plugin --profile novelnovel add ./dsh-plugin

# 3. 验证：在真实 harness 服务上跑通全部工具
npm run test:dsh                 # 默认 profile: novelnovel
DSH_PROFILE=web npm run test:dsh # 指定其它 profile
```

想装到已有的 `web` profile，把第 2 步的 profile 名换成 `web` 即可。
需要重新分发时用 `npm pack dsh-plugin`（在 `dsh-plugin/` 下执行 `npm pack`）产出的 tarball 安装，
这样插件会被复制进 profile 的 `node_modules`，不依赖本仓库工作树。
...
`novel_context` 返回的系统提示词区块是设定与文风的权威来源：它就是浏览器端「续写」用的
system prompt，只是改由 harness 自己的模型来遵守。
...
- **破坏性操作**：删除作品 / 章节 / 角色卡都需要 `confirm=true`，插件会拒绝未确认的调用，
  提示先与用户确认（与浏览器端的两步确认一致）。

## 开发

```bash
npm run build:plugin      # esbuild 打包到 dsh-plugin/lib/
npm run typecheck:plugin  # tsc -p dsh-plugin（严格模式，零错误）
npm run test:dsh          # 端到端验证（需要已安装的 profile）
```

`npm run test:dsh` 会拉起真实 harness 服务（SystemPrompt + ToolRuntime + LocalFileSystem +
SkillRegistry）并驱动全部工具：作品/章节/词条/角色卡（含 PNG 双写回读）/预设/简报组装/
关键词注入命中与未命中/检索/导出/技能注册/命令处理器/配置校验/工作区边界与损坏文件容错/卸载清理，
共 34 项检查，不调用模型。

`dsh-plugin/tests/perf-probe.mjs` 是性能探针（在 profile 目录里跑）：铺 3 部 × 200 章，
