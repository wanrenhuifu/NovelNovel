/**
 * novel_context：写作上下文组装（浏览器版「续写」拼装逻辑的服务端对应物）。
 *
 * harness 自己就是写作模型，所以这里不调用 LLM，而是把「作者设定 + 命中词条 +
 * 参与角色 + 预设模板 + 前文摘录 + 本章结尾」组装成一份写作简报交给 agent，
 * 与浏览器端 buildSystemPrompt / buildContinueUserMessage 走的是同一份纯逻辑。
 */
import { buildContinueUserMessage, buildSystemPrompt, selectLoreEntries } from "../../../src/lib/prompt";
  ...
  ctx.tools.register(
    defineTool({
      ...
      async execute(args, exec) {
        ...
        // 前文摘录也参与 lorebook 关键词匹配（与浏览器端一致）
