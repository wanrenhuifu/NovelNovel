 ...
 *
 * 与浏览器版（IndexedDB）的对应关系：Project/Chapter/Character/LoreEntry/Preset
 * 的字段语义保持一致，只是把主键换成字符串 id、把 Blob 头像换成 PNG 文件，
 * 提示词组装/搜索/排序/导出等纯逻辑直接复用 src/lib，避免两份实现漂移。
 */
import type { LoreEntry, Preset } from "../../src/types";
import { parseCharacterBytes } from "../../src/lib/cardImport";
import { buildCharacterPng, buildNovelDocument, safeName } from "../../src/lib/export";
import { parsePresetFile } from "../../src/lib/presetImport";
import { computeReorder } from "../../src/lib/reorder";
import { searchChapters, type SearchMatch } from "../../src/lib/search";
import { countWords, uid } from "../../src/lib/utils";
...
/** 头像落盘用的扩展名：媒体类型 → 扩展名，未知类型按 PNG（浏览器端同样只认 PNG 嵌卡） */
...
export class NovelStore {
  ...
  /** 移动章节：复用浏览器端 computeReorder 的排序语义（sortOrder 归一化为 0..n-1） */
  async moveChapter(
    session: FsSession,
    projectId: string,
    chapterId: string,
